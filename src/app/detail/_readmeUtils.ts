// ─── README WebView 渲染工具（内联 marked + highlight.js，零 CDN 依赖）──────────
// marked.js v18 UMD + highlight.js core + 12种常用语言，完全离线可用
...
// ── 内联 highlight.js core ───────────────────────────────────────────────────
const HLJS_CORE = `/* eslint-disable no-multi-assign */
...
// ── GitHub 风格 highlight.js CSS ──────────────────────────────────────────────
const HLJS_CSS = `.hljs{color:#24292e;background:#f6f8fa}
...
// ── GitHub 风格 Markdown CSS ──────────────────────────────────────────────────
export const README_CSS = `* { box-sizing: border-box; }
...
body {
  ...
  padding: 4px 0 12px; word-wrap: break-word; overflow-wrap: break-word;
}
...
code {
  ...
  font-size: 85%; background: rgba(175,184,193,0.2); border-radius: 3px; padding: .2em .4em;
}
pre {
...
table { border-collapse: collapse; width: 100%; max-width: 100%; margin: 8px 0 14px; display: block; overflow-x: auto; }
th, td { border: 1px solid #d8dee4; padding: 6px 12px; text-align: left; }
th { background: #f6f8fa; font-weight: 600; }
tr:nth-child(even) td { background: #f8f9fa; }
blockquote { margin: 0 0 14px; padding: 2px 14px; color: #656d76; border-left: 4px solid #d8dee4; }
...
// ── 高度上报脚本 ───────────────────────────────────────────────────────────────
const HEIGHT_SCRIPT = `function getH(){
  var el=document.getElementById('md');
  var c=[document.documentElement.scrollHeight,document.documentElement.offsetHeight,document.body.scrollHeight,document.body.offsetHeight];
  if(el){var r=el.getBoundingClientRect();c.push(Math.ceil(r.bottom+(window.scrollY||0)),el.scrollHeight,el.offsetHeight);}
  return Math.max.apply(null,c.filter(function(v){return v>0;}));
}
function report(){
  var h=getH(); if(h<=0)return;
  var m=JSON.stringify({type:'height',height:h});
...
[80,300,700,1400,2500].forEach(function(t){setTimeout(report,t);});
window.addEventListener('load',function(){setTimeout(report,200);});
if(typeof ResizeObserver!=='undefined'){
  var ro=new ResizeObserver(function(){setTimeout(report,60);});
  var el=document.getElementById('md'); if(el)ro.observe(el); ro.observe(document.body);
}`;

// ── 渲染脚本（依赖 marked/hljs 已就绪）────────────────────────────────────────
const RENDER_SCRIPT = `(function(){
  // 1. Decode markdown from base64
  var raw=decodeURIComponent(escape(atob(B64)));

  // 2. Resolve relative URLs
  raw=raw.replace(/(!\[[^\]]*\]\()((?!https?:\/\/|data:)[^)]+)(\))/g,function(m,a,p,c){return a+BASE+p+c;});
  raw=raw.replace(/(<img[^>]+src=")((?!https?:\/\/|data:)[^"]+)(")/gi,function(m,a,p,c){return a+BASE+p+c;});
  raw=raw.replace(/(<a[^>]+href=")((?!https?:\/\/|mailto:|#|ftp:|\/)[^"]+)(")/gi,function(m,a,p,c){return a+BASE+p+c;});

  // 3. marked config
  marked.use({gfm:true,breaks:false});
  var renderer=new marked.Renderer();
  renderer.code=function(tok){
    var code=typeof tok==='object'?tok.text:tok;
    var lang=typeof tok==='object'?(tok.lang||''):'';
    var lang0=(lang||'').split(/[\s,]/)[0];
    var hl='';
    if(lang0&&hljs.getLanguage(lang0)){
      try{hl=hljs.highlight(code,{language:lang0,ignoreIllegals:true}).value;}
      catch(e){hl=hljs.highlightAuto(code).value;}
    }else{hl=hljs.highlightAuto(code).value;}
    return '<pre><code class="hljs language-'+(lang0||'plaintext')+'">'+hl+'</code></pre>';
  };

  // 4. GitHub Admonitions
  ...
  html=html.replace(/(<img[^>]+src=")([^"]*shields\.io[^"]*|[^"]*badge\.svg[^"]*|[^"]*badgen\.net[^"]*)(")/gi,function(m,a,src,c){
    if(src.indexOf('format=png')>=0)return m;
    return a+(src.indexOf('?')>=0?src+'&format=png':src+'?format=png')+c;
  });
  ...
  // 9. Remove oversized width attrs
  document.querySelectorAll('#md img').forEach(function(img){
    var w=img.getAttribute('width')||'';
    var wn=parseFloat(w);
    if(w.indexOf('%')===-1&&wn>VPW){img.removeAttribute('width');img.removeAttribute('height');}
    if(img.style.width&&img.style.width.indexOf('%')===-1&&parseFloat(img.style.width)>VPW){img.style.width='';img.style.height='';}
  });

  // 10. Image onload height report
  document.querySelectorAll('#md img').forEach(function(img){
    if(!img.complete){
      img.addEventListener('load',function(){setTimeout(report,60);});
      img.addEventListener('error',function(){setTimeout(report,60);});
    }
  });
})();`;

/**
 * 构建用于 WebView 的完整 HTML（零外部依赖，marked + hljs 全部内联）
 */
...
<style>${README_CSS}${HLJS_CSS}</style>
</head>
<body>
<div id="md"></div>
<script>
